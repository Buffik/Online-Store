...
function App() {
  ...
  const [fetchProductsById, isPending] = useFetching(async () => {
    const response = await PostService.getCartItems(productsInCartCount)
  ...
  const decreaseProductCount = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const result = productsInCartCount.reduce<TProductPartialProps[]>((acc, product) => {
      if (product.id === cur && product.count === 1) {
        setProductsInCartCount(productsInCartCount.filter((el) => el.id !== cur));
        return acc;
      }
      if (product.id === cur) {
  ...
  const isProductInCart = (id?: number): boolean => productsInCartCount.some(
    (item) => item.id === id,
  );
